...
async function callChromeMethod(target, methodName, args = []) {
...
export async function requestLocalFontList(chromeApi = globalThis.chrome) {
  ...
  if (typeof permissions.contains === "function") {
    try {
      granted = Boolean(await callChromeMethod(permissions, "contains", [permissionRequest]));
    } catch {
      // contains 在旧版或测试桩中可能不存在/失败，继续走显式申请。
    }
  }
  if (!granted) {
    ...
    try {
      granted = Boolean(await callChromeMethod(permissions, "request", [permissionRequest]));
    } catch {
