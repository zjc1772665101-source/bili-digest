...
export const TYPOGRAPHY_LIMITS = Object.freeze({
  brandFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  titleFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  navigationFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  controlFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  overviewButtonFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  videoActionButtonSize: Object.freeze({ min: 36, max: 80, step: 2 }),
  metaFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  codeFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  readingFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  transcriptFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  overviewFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  notesFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  chatFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
  settingsFontSize: Object.freeze({ min: 12, max: 30, step: 1 }),
...
/**
 * 在用户点击“读取本机字体”后枚举本机字体。
 * 扩展环境优先使用 chrome.fontSettings.getFontList()：它是 Chrome 扩展的稳定系统字体接口。
 * queryLocalFonts() 仅作为备用；部分 Chrome/扩展页面会“成功返回空数组”，不能把空数组当成最终结果。
 */
export async function requestLocalFontList(chromeApi = globalThis.chrome, options = {}) {
  ...
  if (typeof fontSettings?.getFontList === "function") {
    try {
      ...
      if (Array.isArray(fonts) && fonts.length > 0) {
        return { supported: true, granted: true, fonts, error: "" };
      }
      // 极少数环境可能返回空列表；继续尝试 Local Font Access，而不是误报“已读取 0 个”。
    } catch (error) {
      // fontSettings 失败时继续尝试 Local Font Access，最后再返回更有用的错误。
    }
  }
  ...
  if (typeof localFontQuery === "function") {
    try {
      const records = await localFontQuery.call(localFontQueryOwner);
      const fonts = mapLocalFontRecords(records);
      if (fonts.length > 0) {
        return { supported: true, granted: true, fonts, error: "" };
      }
      if (typeof fontSettings?.getFontList === "function") {
        return {
          ...
          granted: true,
          fonts: [],
          error: "Chrome 返回了空的本机字体列表；请在 chrome://extensions 中重新加载扩展后再试。",
        };
      }
      return {
        supported: true,
        granted: true,
        fonts: [],
        error: "Chrome 的 Local Font Access 返回了空列表；当前环境未暴露 fontSettings 备用接口。",
      };
    } catch (error) {
      if (typeof fontSettings?.getFontList === "function") {
        try {
  ...
  if (typeof fontSettings?.getFontList === "function") {
    try {
      ...
      return {
        ...
        error: Array.isArray(fonts) && fonts.length > 0 ? "" : "Chrome 返回了空的本机字体列表。",
      };
    } catch (error) {
      return { supported: true, granted: false, fonts: [], error: `读取本机字体列表失败：${describeFontError(error)}` };
    }
  }
  ...
  return {
    ...
    granted: false,
    fonts: [],
    error: "当前浏览器没有提供可用的本机字体读取接口；请确认扩展已获得 fontSettings 权限并重新加载。",
  };
}
